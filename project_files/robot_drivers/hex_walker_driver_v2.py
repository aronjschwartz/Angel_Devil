...
class Leg(object):
	...
	def set_servo_angle_thread(self, angle: float, servo: int, durr: float):
		...
		L = Leg_Position(v[ROT_SERVO], v[MID_SERVO], v[TIP_SERVO])
...
class Hex_Walker(object):
		...
		for i in range(repeat):
			for next_pose in hex_walker_position_list:
				...
				if next_pose_idx in HEX_WALKER_POSITIONS[self.current_pose].safe_moves:
					...
					print("ERR: invalid move, " + str(self.current_pose) + " -> " + str(next_pose_idx))
					return ILLEGAL_MOVE
		return SUCCESS
	...
	def walk(self, num_steps, front=DIR_F, scale=1.0, durr=None):
		...
		if num_steps == 0:
			return SUCCESS
		if num_steps < 0:
			print("ERR: walk() accepts only num_steps >= 0")
			return INV_PARAM
		...
		if USE_NEW_WALK_CYCLE:
			# TODO: make this walk cycle not be shit, front leg is collapsing when moving forward
			# poses for Brian's new animations
			...
			# poses for original walk animations
		...
		# TODO: test that this actually does sensible fine-stepping with  walk-cycle, its not as obvious as with the rotate cycle
		temp_both = []
	...
	def crab_walk(self, num_steps, direction, front=DIR_F, scale=1.0, durr=None):
		...
		# TODO: completely overhaul this animation sequence
		...
		# NEW: scale the "rot-servo" portion to produce fine-stepping behavior!
		# TODO: pretty sure this won't actually work since the "center" leg isn't being scaled at all
		temp_both = []
		for pose_idx in temp_left_step:  # combine the lists for easier iteration
			pose = HEX_WALKER_POSITIONS[pose_idx].copy()  # dereference and copy
			for l in pose.list:  # modify: for each leg in the hex-pose, scale the rot-servo around 90*
				l.list[ROT_SERVO] = ((l.list[ROT_SERVO] - 90.) * scale) + 90.  # subtract 90, scale, add 90
			temp_both.append(pose)  # store
	...
	# fine_rotate is just here for legacy support
